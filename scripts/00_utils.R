  ...
  for (i in seq_len(nrow(week_meta))) {
    ...
    out_list[[i]] <- draw_tbl %>%
      ...
      summarise(
        ...
        q25 = quantile(bt_logit_score, 0.25),
  ...
  for (i in seq_len(nrow(week_meta))) {
    ...
    out_list[[i]] <- draw_tbl %>%
      ...
      summarise(
        ...
        q25 = quantile(weighted_severity_logit_score, 0.25),
...
bootstrap_bt_end_to_end_win <- function(
  ...
  validation_summary <- metric_draws %>%
    ...
    summarise(
      ...
      improvement_q25 = quantile(improvement, 0.25),
      improvement_q50 = quantile(improvement, 0.50),
      improvement_q75 = quantile(improvement, 0.75),
...
bootstrap_player_rating_uncertainty <- function(model_data, model_cfg, n_boot, seed = 42L, workers = 1L) {
  ...
  bind_rows(draws) %>%
    ...
    summarise(
      ...
      q25 = quantile(final_elo, 0.25),
      q50 = quantile(final_elo, 0.50),
      q75 = quantile(final_elo, 0.75),
