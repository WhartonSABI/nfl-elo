...
```{r}
#week block permutation function
...
#rowwise resampler function
run_row_resample_elo <- function(data, row_index, initial_elo_vec, iter_id = NA_integer_) {
...
make_row_resamples <- function(n, B, replace = FALSE) {
  replicate(
    B,
    sample.int(n = n, size = n, replace = replace),
    simplify = FALSE
  )
}
```
...
B <- 10000
n_rows <- nrow(results_cleaned)

# toggle:
# FALSE = row permutation
# TRUE  = row bootstrap
resample_with_replacement <- TRUE

row_resamples <- make_row_resamples(
  n = n_rows,
  B = B,
...
elo_bootstrap_results <- future_map2_dfr(
  .x = row_resamples,
  .y = seq_along(row_resamples),
  .f = ~ run_row_resample_elo(
    data = results_cleaned,
    row_index = .x,
    initial_elo_vec = initial_elo_vec,
    iter_id = .y
...
```{r}
data.table::fwrite(elo_bootstrap_results, "~/Downloads/elo_rowwise_bootstrap_raw.csv")
```

```{r}
data.table::fwrite(elo_summary, "~/Downloads/elo_rowwise_bootstrap_summary.csv")
```
