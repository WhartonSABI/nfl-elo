...
blocker_counts <- matchups_all %>%
  group_by(game_id, play_uuid, rusher_id) %>%
  summarize(n_blockers_for_rusher = n(), .groups = "drop")
...
safe_eval <- function(rusher, blocker, start, end, pass_time, tracking,
                      game_id, play_id, event_game_index, n_blockers_for_rusher) {
  tryCatch({
    if (is.na(start) || is.na(end) || is.na(pass_time) || !is.data.frame(tracking)) {
      return(tibble(
        ...
        beat_dist    = NA_real_,
        rusher_won   = 0L, 
        n_blockers_for_rusher = n_blockers_for_rusher
      ))
    }
...
results2 <- future_pmap_dfr(
  list(
    ...
    n_blockers_for_rusher = matchups_all_nested$n_blockers_for_rusher,
