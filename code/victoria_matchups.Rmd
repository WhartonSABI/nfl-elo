...
pacman::p_load(tidyverse, dplyr, ggthemes, data.table, lubridate, glmnet,
               ...
               tidymodels,ranger,vip,ggplot2, tune,dials,pdp, purrr, stringr, lmtest, future, furrr,
               sandwich)
...
# filter for vickysuzie
new_data <- new_data %>%
  filter(week %in% 1:12)

#unique(new_data$week)
```
...
write.csv(sacks, '../data/results/sacks.csv', row.names = FALSE)
```
...
write.csv(hits, '../data/results/hits.csv', row.names = FALSE)
```

Penalties
```{r}

unique(new_data$event_penalty_type)
block_pens <- c("Offensive Holding", "Illegal Use of Hands", "Chop Block", 
                "Face Mask (15 Yards)", "Clipping")

rush_pens <- c("Illegal Use of Hands", "Face Mask (15 Yards)", "Defensive Holding")

penalty_flags <- new_data %>%
  filter(!is.na(event_penalty_type)) %>%
  mutate(event_penalty_type = str_remove_all(event_penalty_type, "[\\{\\}\"]")) %>%
  separate_rows(event_penalty_type, sep = ",\\s*") %>%
  select(game_id, play_uuid, event_player_name, event_penalty_type) %>%
  distinct()

blocker_penalties <- penalty_flags %>%
  filter(event_penalty_type %in% block_pens) %>%
  mutate(blocker_penalty = 1L) %>%
  rename(blocker_name = event_player_name) %>%
  select(game_id, play_uuid, blocker_name, blocker_penalty)

# Rusher penalties
rusher_penalties <- penalty_flags %>%
  filter(event_penalty_type %in% rush_pens) %>%
  mutate(rusher_penalty = 1L) %>%
  rename(rusher_name = event_player_name) %>%
  select(game_id, play_uuid, rusher_name, rusher_penalty)
```

Filter 
```{r}
matchups <- new_data %>% 
  select(freeze_frame_player, event_player_name, opponent_player_name, event_uuid, event_game_index, game_id, play_uuid, player_id,  time_since_snap,opponent_player_id,  opponent_position, freeze_frame_player_id, event_types, event_x, event_y, start_engagement_uuid, end_engagement_uuid, play_start_event, time_since_snap, freeze_frame_x, freeze_frame_y, event_player_position, freeze_frame_player_id, freeze_frame_position, freeze_frame_player_team, nflfast_game_id, week)
```
...
matchup_tracking <- matchups %>%
  left_join(
    matchups %>% 
      select(game_id, play_uuid, time_since_snap, freeze_frame_player_id, freeze_frame_x, freeze_frame_y, freeze_frame_player, event_game_index, week) %>% 
      rename(
    ...
    by = c("game_id", "play_uuid", "time_since_snap", "opponent_player_id"), relationship = "many-to-many"
  )
...
matchup_tracking_with_qb <- matchups %>%
  left_join(
    ...
    by = c("game_id", "play_uuid", "time_since_snap"), relationship = "many-to-many"
  )
```
...
matchups_all <- engagements %>%
  ...
  arrange(week, play_uuid, event_game_index, player_id, opponent_player_id, event_types) %>%
  ...
  summarize(
  rusher_idx = which(event_player_position %in% c("Defensive Lineman", "Linebacker"))[1],
  ...
  rusher_name = if (!is.na(rusher_idx)) event_player_name[rusher_idx] else NA_character_,
  ...
  event_game_index = min(event_game_index, na.rm = TRUE),
...
Get double teams
```{r}
double_team_pairs <- matchups_all %>%
  inner_join(matchups_all, by = c("game_id", "play_uuid", "rusher_id"), suffix = c("_1", "_2")) %>%
  filter(blocker_id_1 != blocker_id_2) %>%
  filter(
    Engage_start_1 < Engage_end_2 &
    Engage_start_2 < Engage_end_1
  ) %>%
  select(game_id, play_uuid, rusher_id, blocker_id_1, blocker_id_2)

double_team <- double_team_pairs %>%
  pivot_longer(cols = starts_with("blocker_id"), values_to = "blocker_id") %>%
  distinct(game_id, play_uuid, rusher_id, blocker_id) %>%
  group_by(game_id, play_uuid, rusher_id) %>%
  summarize(double_team = 1L, .groups = "drop")

matchups_all <- matchups_all %>%
  add_count(game_id, play_uuid, rusher_id, name = "n_blockers_for_rusher") %>%
  mutate(double_team = if_else(n_blockers_for_rusher > 1, 1L, 0L))

# matchups_all <- matchups_all %>%
#   left_join(blocker_counts, by = c("game_id", "play_uuid", "rusher_id")) %>%
#   left_join(double_team, by = c("game_id", "play_uuid", "rusher_id")) %>%
#   mutate(double_team = replace_na(double_team, 0L))
```
...
matchups_all_nested <- matchups_all_nested %>%
  arrange(game_id, event_game_index)
...
safe_eval <- function(rusher, blocker, start, end, pass_time, tracking,
                      game_id, play_id, event_game_index, double_team) {
  tryCatch({
...
results2 <- future_pmap_dfr(
  list(
    ...
    double_team = matchups_all_nested$double_team,
    event_game_index = matchups_all_nested$event_game_index 
  ),
  safe_eval,
...
```{r}
results2 <- results2 %>%
  left_join(rusher_penalties, by = c("game_id", "play_id"="play_uuid", "rusher_name")) %>%
  left_join(blocker_penalties, by = c("game_id", "play_id"="play_uuid", "blocker_name")) %>%
  mutate(
...
results2 <- results2 %>%
  mutate(
    rusher_won = case_when(
      rusher_won == 1L & rusher_penalty == 1L ~ 0L,  # Rusher won but committed a penalty → switch to loss
      rusher_won == 0L & blocker_penalty == 1L ~ 1L, # Blocker won but committed a penalty → switch to rusher win
      TRUE ~ rusher_won
    )
  )
...
Find win freq of double teams
```{r}
double_team_win_freq <- results2 %>%
  group_by(double_team) %>%
  summarise(
    win_freq = mean(rusher_won, na.rm = TRUE),
    total_engagements = n(),
    .groups = "drop"
  )
```
...
```{r}
write.csv(results2, '../data/results/results2.csv')
```