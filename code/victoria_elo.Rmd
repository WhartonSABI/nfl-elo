...
```{r}
results2 = fread('../data/processed/results2.csv')
head(results2)
sacks <- read.csv('../data/processed/sacks.csv')

hits <- read.csv('../data/processed/hits.csv')
...
elo_step <- function(
  ...
  c(
    b = belo + blocker_K * ((1 - outcome_score) - (1 - exp_rusher)),  # blocker reward
    r = relo + rusher_K  * ( outcome_score      -      exp_rusher)   # rusher reward
  )
}
...
```
```{r}
pro_bowl = c("T.J. Watt", "Myles Garrett")
