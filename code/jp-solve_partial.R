# this script runs the elo rating system on partial datasets
...
# read in data
new_data <- fread("../data/processed/five_weeks_hudl_data.csv")
...
top_10_rushers = ggplot(elo_top_rusher, aes(x = as.numeric(interaction_n), y = elo, color = player_name)) +
  geom_line(size = 1) +
  labs(
    title = "Top 10 Rushers: Elo Trajectory (Partial)",
...
top_10_blockers = ggplot(elo_top_blocker, aes(x = as.numeric(interaction_n), y = elo, color = player_name)) +
  geom_line(size = 1) +
  labs(
    title = "Top 10 Blockers: Elo Trajectory (Partial)",
    x = "Interaction Number",
    y = "Elo Rating",
    color = "Blocker"
  ) +
...
write_csv(elo_history, "../data/results/partial_elo_history.csv")
write_csv(elo_labeled, "../data/results/partial_player_elo_ratings.csv")
# save plots
ggsave("../data/results/partial_top_10_rushers.png", plot = top_10_rushers, width = 8, height = 6)
ggsave("../data/results/partial_top_10_blockers.png", plot = top_10_blockers, width = 8, height = 6)
